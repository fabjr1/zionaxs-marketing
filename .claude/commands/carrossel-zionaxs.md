...
1. **Verifique a identidade da conta ao vivo** com `INSTAGRAM_GET_USER_INFO`. Há três contas conectadas e só `zionaxs_` (id `37965311306447572`, account `instagram_ascent-utick`) é a certa. Conta divergente aborta tudo.
   **O `account` do Composio é o apelido da conexão, e o apelido tem arroba: `@zionaxs_`.** Passar `zionaxs_` sem arroba volta `No account found matching "zionaxs_"`. Aprendido na zx-51, 13/09/2026, e custou 1 rodada. Quando houver dúvida, leia os apelidos ao vivo com `COMPOSIO_MANAGE_CONNECTIONS` em modo `list` sobre o toolkit `instagram`. Vale para **todas** as chamadas do fluxo, não só a primeira.
   **E o `account` deixou de ser opcional por um motivo novo: a conexão padrão do toolkit não é mais a Zionaxs.** Aprendido na zx-53, 14/09/2026. A leitura ao vivo devolveu 4 conexões, e não 3: `@clubegarimpai`, `@fabjr1`, `@zionaxs_` e `@base3br`. A primeira foi criada em 13/09/2026 e está marcada como `is_default: true`. Ou seja, uma chamada que omita `account` hoje publica na conta errada, sem erro nenhum e sem aviso. Antes disso o padrão era inofensivo, porque a conta padrão era a da própria operação. **Confira a lista de conexões a cada execução e nunca omita o `account`**, mesmo em chamada de leitura, e confirme a identidade com `INSTAGRAM_GET_USER_INFO` antes de criar o primeiro contêiner: o id tem de ser `37965311306447572` e o username `zionaxs_`.
...
> **Erro de validação do Composio não é erro de publicação.** As 2 falhas acima acontecem **antes** de a chamada tocar a API da Meta, então nada foi criado e não há o que reconciliar: corrija o nome do campo e repita. Só falha de transporte e pai em `ERROR` pedem reconciliação antes de repetir.
...
7. **Reconcilie sempre**: `INSTAGRAM_GET_IG_MEDIA` e `INSTAGRAM_GET_IG_MEDIA_CHILDREN` confirmando conta, permalink, tipo e número de slides. **Chame o `INSTAGRAM_GET_IG_MEDIA` já com `fields` explícito**, `id,caption,media_type,permalink,shortcode,timestamp,username,owner`: a lista padrão do Composio pede `total_views_count`, que o nó Media não suporta, e a chamada volta HTTP 400 com o post já no ar. Aprendido na zx-40, 07/09/2026. A falha é só de leitura e não desfaz a publicação, mas assusta no meio da reconciliação e custa uma rodada.
